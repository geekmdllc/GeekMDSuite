...
namespace GeekMDSuite.WebAPI.Presentation.Controllers.AnalyzablePatientDataControllers
{
    ...
    public class SitupsController : VisitDataController
    {
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                var entity = await UnitOfWork.Situps.FindById(id);
                var stub = Mapper.Map<SitupsEntity, SitupsStub>(entity);
                var resource = new SitupsResource
                {
            ...
            catch (RepositoryEntityNotFoundException)
            {
                var error = ErrorService.PayloadBuilder
                    ...
                    .HasInternalMessage($"An situps entity with the id {id} could not be located in the repository.")
                    .TellsUser("The requested situps entry could not be found")
                    .Build();
                return BadRequest(error);
            }
        }

        public async Task<IActionResult> Post([FromBody] SitupsStubFromUser stub)
        {
            try
            {
                var entity = Mapper.Map<SitupsStubFromUser, SitupsEntity>(stub);
                await UnitOfWork.Situps.Add(entity);
                await UnitOfWork.Complete();
                var url = Url.Action<SitupsController>(a => a.Post(stub));
            ...
            catch (ArgumentNullException)
            {
                var error = ErrorService.PayloadBuilder
                    ...
                    .HasInternalMessage(
                        "A null object was included in the request body and cannot be processed as an situps entity")
                    .TellsUser(
                        "The request to create a new situps entry was malformed and likely empty. Please retry.")
                    .Build();
                return BadRequest(error);
            }
            catch (EntityNotUniqeException)
            {
                var error = ErrorService.PayloadBuilder
                    ...
                    .TellsUser("The request to create a new situps entry was imporoperly formatted ")
                    .Build();
        ...
        public async Task<IActionResult> Put(int id, [FromBody] SitupsStubFromUser stub)
        {
            ...
            {
                var entity = Mapper.Map<SitupsStubFromUser, SitupsEntity>(stub);
                await UnitOfWork.Situps.Update(entity);
                await UnitOfWork.Complete();
                return Ok(stub);
            }
            catch (RepositoryEntityNotFoundException)
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                await UnitOfWork.Situps.Delete(id);
                await UnitOfWork.Complete();
                return NoContent();
            }
            catch (RepositoryEntityNotFoundException)
        ...
        private IEnumerable<SitupsResource> GenerateSitupsResources(IEnumerable<SitupsEntity> entities)
        {
            var stubs = entities.Select(Mapper.Map<SitupsEntity, SitupsStub>);
            var resources = stubs.Select(stub => new SitupsResource
            {
                Properties = stub,
                Links = new List<ResourceLink>
                {
                    new ResourceLink
                    {
                        Description = "Get this situps resource",
                        Href = Url.Action<SitupsController>(a => a.GetById(stub.Id)),
        ...
        private List<ResourceLink> GenerateGetByIdLinks(int id)
        {
            return new List<ResourceLink>
            {
                new ResourceLink
                {
                    Description = "Get situps resource by it's unique identifier.",
                    Href = Url.Action<SitupsController>(a => a.GetById(id)),
                    HtmlMethod = HtmlMethod.Get,
                    Relationship = UrlRelationship.Self
                },
                new ResourceLink
                {
                    Description = "Delete situps resource by it's unique identifier.",
                    Href = Url.Action<SitupsController>(a => a.Delete(id)),
                    HtmlMethod = HtmlMethod.Delete,
                    Relationship = UrlRelationship.Next
                },
                new ResourceLink
                {
                    Description = "Update the values for this existing situps resource",
                    Href = Url.Action<SitupsController>(a => a.Put(id, With.No<SitupsStubFromUser>())),
                    HtmlMethod = HtmlMethod.Put,
                    Relationship = UrlRelationship.Search
                },
                new ResourceLink
                {
                    Description = "Search for Situpss resources with filters and pagination",
                    Href = Url.Action<SitupsController>(a => a.GetBySearch(With.No<EntityDataFindFilter>())),
                    HtmlMethod = HtmlMethod.Get,
                    Relationship = UrlRelationship.Search
                },
                new ResourceLink
                {
                    Description = "Add a new situps resource",
                    Href = Url.Action<SitupsController>(a => a.Post(With.No<SitupsStubFromUser>())),
