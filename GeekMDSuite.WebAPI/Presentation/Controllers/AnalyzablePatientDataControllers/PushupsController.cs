...
namespace GeekMDSuite.WebAPI.Presentation.Controllers.AnalyzablePatientDataControllers
{
    ...
    {
        public PushupsController(IUnitOfWork unitOfWork, IMapper mapper, IErrorService errorService) : base(unitOfWork,
            mapper, errorService)
        {
        }
        ...
        public async Task<IActionResult> Put(int id, [FromBody] PushupsStubFromUser stub)
        {
            if (stub != null && id != stub.Id)
            {
                var error = ErrorService.PayloadBuilder
                    ...
                    .HasInternalMessage(
                        $"There endpoint targeted a pushups entity with Id {id}, but the pushups resource object contained Id {stub.Id}")
            ...
            catch (RepositoryEntityNotFoundException)
            {
                var error = ErrorService.PayloadBuilder
                    .HasErrorCode(ErrorPayloadErrorCode.RepositoryEntityNotFound)
                    .HasInternalMessage(
                        $"An pushups entity with Id {id} could not be located in the repository. No changes were made.")
                    .TellsUser(
                        "The request could not be processed because the pushups entry identified for update couldn't not be found")
                    .Build();
                return BadRequest(error);
            }
            catch (ArgumentNullException)
            {
                var error = ErrorService.PayloadBuilder
                    .HasErrorCode(ErrorPayloadErrorCode.DataModelFromUserIsInvalid)
                    .HasInternalMessage(
                        "The pushups data model received from client was null and could not be processed.")
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            catch (RepositoryEntityNotFoundException)
            {
                var error = ErrorService.PayloadBuilder
                    .HasErrorCode(ErrorPayloadErrorCode.RepositoryEntityNotFound)
                    .HasInternalMessage(
                        $"An pushups element with Id {id} could not be located in the repository. No changes were made.")
